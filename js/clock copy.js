  ...
  clock_box_html = `
    <div class="clock-row">
      ...
      <span class="card-clock-weather">
        <i class="qi-${e.now.icon}-fill" style="color: ${a}"></i> 
        ${e.now.text || currentLang.unknownWeather} 
        <span class="temperature">${e.now.temp} ℃</span>
      </span>
    ...
    <div class="clock-row">
      <span class="card-clock-windDir"> 
        <i class="qi-gale"></i> ${e.now.windDir || "-"}
      </span>
...
function initElectricClock() {
  ...
  const defaultCityZh = "哈尔滨";  // 修改为哈尔滨
